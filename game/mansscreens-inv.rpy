...
label mansSheath:
    scene bg mansiondining knife at bg
...
label mansDracula:
    ...
    if False not in mans_evidence[7:9]:
        bi "Hmm... I think that's everything to find here."
        bi "Not that I found much, mostly the sheath and talking to Dracula about stuff I already knew..."
    call mansDone from _call_mansDone
    call screen diningInv
...
screen bedroomInv():
    ...
    imagemap:
        ground "bg mansionbedroom2.png"
        if mans_extra[10]:
            hotspot(917, 480, 183, 131) action [Hide("bedroomInv"), Jump("mansGoAway")] mouse 'q' hovered tt.Action("Nightstand and Flower")
        else:
            hotspot(917, 480, 183, 131) action [Hide("bedroomInv"), Jump("mansGoAway")] mouse 'ex' hovered tt.Action("Nightstand and Flower")
    if tt.value != "":
        frame:
            xalign 0.5
            yalign 0.0
...
label mansGoAway:
    ...
    #    "Bert checked the other third small bedroom."
    #    bi "...Nothing here."
    #    bi "That just leave's Jenny's room..."
    #    scene bg mansionbedroom1gen at bg with dissolve
    #    $ statusnt("Bedroom", "bert", ch = 2, sun = 3)
    #    bi "...Huh?"
...
label mansGenerator:
    scene bg mansionbedroom1gen at bg
    $ statusnt("Bedroom", "bert", ch = 2, sun = 3)
    if not mans_evidence[5]:
        ...
        bi "Hm..."
    call mansDone from _call_mansDone_1
...
label mansCath:
    scene bg mansionmasterbedroom at bg
...
label mansSink:
    ...
    call mansDone from _call_mansDone_2
...
label mansHands:
    ...
    if False not in mans_evidence[0:5]:
        bi "Hmm... I found a lot in the bathroom, but I think that's everything."
        bi "Though this has brought up more questions than answers..."
    call mansDone from _call_mansDone_3
    call screen bathroomInv
...
label mansStella:
    ...
    if False not in mans_evidence[0:5]:
        bi "Hmm... I found a lot in the bathroom, but I think that's everything."
        bi "Though this has brought up more questions than answers..."
    call mansDone from _call_mansDone_4
    call screen bathroomInv
